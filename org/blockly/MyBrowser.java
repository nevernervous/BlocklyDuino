...
public class MyBrowser extends Region {
	...
	static RadioButton arduinoButton = new RadioButton("Scratch");
	static RadioButton scratchButton = new RadioButton("Arduino");
	...
	static final Browser  scratchBrowser = new Browser();
	static final WebEngine webEngine = browser.getEngine();
	static final BrowserView scratchView = new BrowserView(scratchBrowser);
	
	private static Boolean bMyBrowser = false;
	...
	public MyBrowser(Stage stage) {
		...
		browser.setVisible(bMyBrowser);
		scratchView.setVisible(!bMyBrowser);
		scratchBrowser.loadURL(scratchPath);
		below.getChildren().addAll(new Node[] { toolBar, output_text });
		sp1.getChildren().addAll(new Node[] { scratchView, browser});
